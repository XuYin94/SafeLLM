...
def train_one_epoch_with_robot(
    args,
    model,
    epoch,
    robot_loader,
    ...
    # setup loaders
    num_batches_per_epoch_mmc4 = robot_loader.num_batches
    num_batches_per_epoch = num_batches_per_epoch_mmc4
    total_training_steps = num_batches_per_epoch * args.num_epochs
    ...
    # loop through dataloader
    for num_steps, batch_robot in tqdm(
        enumerate(robot_loader),
        disable=args.rank != 0,
        total=total_training_steps,
        initial=(epoch * num_batches_per_epoch),
    ):
        ...
        #### MMC4 FORWARD PASS ####
        images = batch_robot[0].to(device_id, dtype=cast_dtype, non_blocking=True)
        input_ids = torch.stack([x[0] for x in batch_robot[1]]).squeeze(1)
        attention_mask = torch.stack([x[1] for x in batch_robot[1]]).squeeze(1)
        ...
        divided_loss_robot = loss_robot / args.gradient_accumulation_steps
        divided_loss_robot.backward()
        ...
        if (((num_steps + 1) % args.gradient_accumulation_steps) == 0) or (
            ...
            # rank 0 logging
            if args.rank == 0 and args.report_to_wandb:
                robot_samples_per_second = (
                    args.gradient_accumulation_steps
                    * args.batch_size_robot
                    * args.world_size
                    / step_time_m.val
                )
                robot_samples_per_second_per_gpu = (
                    args.gradient_accumulation_steps
                    * args.batch_size_robot
                    / step_time_m.val
                )
                wandb.log(
                    {
                        "data_time": data_time_m.avg,
                        "step_time": step_time_m.avg,
                        "robot_samples_per_second": robot_samples_per_second,
                        "robot_samples_per_second_per_gpu": robot_samples_per_second_per_gpu,
                        "lr": optimizer.param_groups[0]["lr"],
                    },
                    commit=False,
                )
                ...
                wandb.log(
                    {"loss_robot": loss_robot.item(), "global_step": global_step},
                    commit=True,
                )
        ...
        if ((num_steps + 1) % args.logging_steps == 0) and args.rank == 0:
            print(
                f"Step {num_steps+1}/{num_batches_per_epoch} of epoch {epoch+1}/{args.num_epochs} complete. Loss MMC4: {loss_robot.item():.3f}"
            )