...
def preprocess_robot_data(sample, tokenizer,max_tokens=256):
    """
    ...
    valid_images=[]
    for i, sample_image in enumerate(image):
        rawbytes = base64.b64decode(sample_image)
    ...
    return (image_tensors, (text_tensor["input_ids"], text_tensor["attention_mask"]))
...
def get_robot_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    """
    Initialize webdataset for ROBOT data
    """
    input_shards = args.robot_shards
    assert input_shards is not None
    resampled = getattr(args, "dataset_resampled", False)
    ...
    num_samples = None
    if not num_samples:
        num_samples = args.train_num_samples_robot
        if not num_samples:
            raise RuntimeError(
                "Currently, number of dataset samples must be specified for training dataset. "
                "Please specify via `--train-num-samples` if no dataset length info present."
    ...
    # create two preprocess functions that take in the passed in image_processor and tokenizer
    preprocess_fn = functools.partial(
        preprocess_robot_data, tokenizer=tokenizer
    )
    # at this point we have an iterator over all the shards
    if not resampled:
        pipeline.extend(
    ...
    pipeline.extend(
        [
            ...
            wds.batched(args.batch_size_robot, partial=False),
        ]
    )
    ...
    # roll over and repeat a few samples to get same number of full batches on each node
    round_fn = math.floor if floor else math.ceil
    global_batch_size = args.batch_size_robot * args.world_size
    num_batches = round_fn(num_samples / global_batch_size)
    num_workers = max(1, args.workers)
    num_worker_batches = round_fn(num_batches / num_workers)  # per dataloader worker
    num_batches = num_worker_batches * num_workers
