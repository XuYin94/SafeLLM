...
from cliport.utils.utils import stack_block_pyramid_anomaly_generator
...
def one_episode_execution(agent,env,task,add_anomaly=False,action_error=False,recording_step=1):
    ...
    for step in range(task.max_steps):
        ...
        obj_id = act['obj_id']
        # color=task.target_block_info[len(processed_objs)][1]
        ...
        if len(processed_objs)==recording_step:
            thread_list=[]
            rgb_list=[]
            depth_list=[]
            rgb,depth= env.multi_view_render()
            ...
            if add_anomaly:
                if act is not None:
                    ...
                    anomaly_generation = threading.Thread(target=stack_block_pyramid_anomaly_generator,
                                                          args=(env, output_queue, task, perturbation_type,len(processed_objs),type))
...
def main(cfg):
    ...
    while save_idx < cfg['n']:
        ...
        env.set_task(task)
        env.reset()
        for recording_step in range (0, task.gt_step-1):
