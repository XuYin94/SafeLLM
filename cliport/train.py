...
def main(cfg):
    ...
    max_epochs = cfg['train']['n_steps'] // cfg['train']['n_demos']#*batch_size
    ...
    trainer = Trainer(
        ...
        check_val_every_n_epoch=min(max_epochs // 5, 5),
    ...
    train_loader = DataLoader(train_ds, batch_size=batch_size, collate_fn=collate_fn, num_workers=8)
    val_loader = DataLoader(val_ds, collate_fn=collate_fn, num_workers=8)

    # Initialize agent
    agent = agents.names[agent_type](name, cfg, train_loader, val_loader,val=False)
