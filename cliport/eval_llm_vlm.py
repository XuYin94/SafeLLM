...
from prompt import general_prompt,cot_prompt_anomaly_handling_pack_boxes,cot_prompt_without_anomaly,cot_prompt_anomaly_handling_pick_place
from cliport import agents
from cliport import dataset
from cliport import tasks
from cliport.environments.environment import Environment
from cliport.utils import utils
from llm import load_llm,get_legal_action_plan,get_next_step_plan
...
def one_step_execution(env,task,agent,obs,info,episode_message,llm_args,use_vlm,step,anomaly,total_reward,vlm_args=None):
    ...
    generation,lang_inst=get_legal_action_plan(history_message=episode_message,return_prompt = True,llm_args= llm_args,feedback=None)
    #print(generation)
    lang_inst=lang_inst.split(':')[1].strip()
    episode_message+=generation
    if lang_inst=="done()":
    ...
    while not output_queue.empty():
        ...
        if not use_vlm: ## use the gt feedback from Pybullet
            #if reward>0:
            feedback="Scene: The action is executed successfully and "+anomaly
        else:
            feedback=get_vlm_feedback(img_obs,lang_inst,vlm_args,device)
        total_reward+=reward
        print("{scene} | Total_reward: {reward}".format(scene=feedback,reward=total_reward))
        episode_message+='\n'+feedback
        anomaly_desc=feedback.split('and')[1][1:-1]
        if not "no anomaly" in anomaly_desc: ## start to handle the anomaly, currently assume that no anomaly would happen during handling process
            ## reasoning the anomaly scene
            episode_message+="\n"+"User: Analyze the effect of the anomaly ['{anomaly}'] on the task regarding progress and feasibility.\n".format(anomaly=anomaly_desc)
            #print(get_next_step_plan(episode_message,False,llm_args,feedback,length=256)[1])
            episode_message+=get_next_step_plan(episode_message,False,llm_args,feedback,length=256)[1]
...
def main(vcfg):
    ...
    prompt=get_prompt(type=vcfg['prompt_type'])
    # VLM
    ...
    for ckpt in ckpts_to_eval:
        ...
        for i in range(0, n_demos):
            ...
            episode_message=prompt+'\n'+"Task: "+final_goal+"\n"+"Initial_state: ["+initial_state+']'+'\n' ## this step would be replaced by the VILD to generate the scene description
            #print(episode_message)
...
def get_prompt(type:str):
    if type=="general":
        ...
        print("fuck")
        return cot_prompt_anomaly_handling_pick_place
