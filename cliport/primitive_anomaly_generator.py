...
def one_episode_execution(info,obs,agent,env,task,episode,output_queue,event,add_anomaly=True):
    for _ in range(task.max_steps):
        ...
        if add_anomaly:
            if 'trash can' in lang_goal:
                return None
            if act is not None:
                type=random.choice(["pick","place","container","miss"])
                anomaly = anomaly_generator_for_primitive(env, task,type=type)
                if anomaly is None:
                    return None
        else:
