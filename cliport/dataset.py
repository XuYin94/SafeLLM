...
class RavensDataset(Dataset):
    ...
    def add(self, seed, episode,font_img_list=None,topdown_img_list=None):
        ...
        dump(reward, 'reward')
        dump(info, 'info')
        if font_img_list is not None:
            dump(font_img_list,'font_img')
        if topdown_img_list is not None:
            dump(topdown_img_list,'topdown_img')
        self.n_episodes += 1
        self.max_seed = max(self.max_seed, seed)
