...
class Environment(gym.Env):
    ...
    def remove_object(self, obj_id, category='rigid'):
        """List of (fixed, rigid, or deformable) objects in env."""
        p.removeBody(obj_id)
    ...
    def step(self, action=None,action_error=False):
        ...
        # Get task rewards.
        reward, info = self.task.reward(self) if action is not None else (0, {})
        done = self.task.done()
    ...
    def get_question(self):
        if self.task:
            return self.task.get_question()
        else:
            raise Exception("No task for was set")

    def get_answer(self):
        if self.task:
            return self.task.get_answer()
        else:
            raise Exception("No task for was set")
    # ---------------------------------------------------------------------------
